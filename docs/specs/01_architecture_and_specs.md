...
3. **セキュリティ境界モデル（クライアント公開キー vs サーバーサイド秘密鍵）**:
   - **対話型地図のクライアント描画必然性**: Google Maps JavaScript API はブラウザ上で動的にマップ要素・マーカーを描画する仕様上、クライアント側（`<APIProvider>`）にキーを渡すことが公式標準アーキテクチャである。
   - **4重防護構造による課金・盗難リスク遮断**:
     - ① **HTTPリファラー制限**: 許可されたドメイン（`*.vercel.app/*` および `localhost:*`）以外からの通信をGoogle Cloud側で403拒否。
     - ② **APIスコープ制限**: `Maps JavaScript API` のみに限定し、高額なGeocoding等の不正呼び出しを遮断。
     - ③ **日次クォータ上限（ハードリミット）**: 1日のリクエスト上限を無料枠内（月$200枠）に設定し、万が一のキー流出時も超過課金をGoogle側で物理遮断。
     - ④ **UIフェイルセーフ**: APIキー未設定・エラー時は「Google Maps デモ表示モード」へ自動縮退し、アプリ全体の停止を防止。
   - **Supabase Anon Key の防護**: クライアントから直接通信する `VITE_SUPABASE_ANON_KEY` は公開前提キーであり、機密保護は PostgreSQL の RLS（行単位セキュリティ）により端末ごとに完全論理分離する。
