...
## 1. 概要
全プロジェクト共通「グローバル運用規約（GEMINI.md）」および「プロジェクト固有ルール（.gemini/rules/project_docs.md）」において、資料間競合防止の恒久ガードレール（Single Source of Truth 多重定義禁止、仕様変更3大トリガーの定義、ペア同期Atomic Commit原則、管轄マップ明示、セッション終了時不可分連動）が確立されました。
また、`docs/` 配下全ドキュメントの記述競合完全排除（スキーマ完全統一、SQL platform追加、参照パス修正、Mermaid最適化、FAQポリシー名是正、テストケース網羅）も完了しています。
資料群の確定仕様に基づき、新セッションではいよいよ**「フェーズ1 iPhone PWA 現場UI改修および堅牢性実装」**に着手します。

## 2. 直近コミット
- コミット: 最新コミット `docs(specs): 全ドキュメントのルール適合性チェックおよび記述競合の完全排除・パレート最適化 [skip ci]`
