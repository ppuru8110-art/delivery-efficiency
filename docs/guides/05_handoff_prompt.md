...
## 1. 概要
全プロジェクト共通「グローバル運用規約（GEMINI.md）」および「プロジェクト固有ルール（.gemini/rules/project_docs.md）」の極限ブラッシュアップ（パレート最適 100点満点）に続き、`docs/` 配下全ドキュメント（要件定義書・設計書群・運用ガイド群・SQLスキーマ）のルール適合性チェックおよび記述競合の完全排除（Single Source of Truth スキーマ完全統一、SQL platformカラム追加、参照パス修正、Mermaid最適化、FAQポリシー名是正、テストケース網羅）が完了しています。
