...
### 5. プロジェクト規約 ＆ 設計仕様（済）
- [x] グローバル運用規約（GEMINI.md）との整合性確立 ＆ プロジェクト固有ルール（.gemini/rules/project_docs.md）の連動最適化完了（PS5.1互換・例外解釈封殺・100dvh・touch-action・バウンス防止/内部スクロール領域保護・Wake Lock・ストレージ名前空間拡張性・upsert冪等性/Last-Write-Wins・本認証移行パス・管理者鍵排除・GPSタイムアウト/手動フォールバック・Vitest将来スコープ規定・FinOpsマスタ取得スコープ適正化・0円運用・ドキュメント同期最適化・LocalStorageパージ基準具体化・Realtime通信抑制・Google Maps多重ロード/インスタンス再生成課金防止・無差別add禁止個別指定・長時間稼働リソース解除/未登録データ破棄厳禁・Sync単一実行排他制御・グローバル規約拡張性強化［言語弾力化/lockfile整合性・再現可能ビルド/外部CDN直リンク排除・バンドル同梱原則/シェル仕様遵守/コンフリクト保護/.env.example同期/環境分離ハードコード禁止/後方互換データ保全/非同期ライフサイクル管理/安全ロールバック（git revert）/再開時lockfile同期確認/縮退運転（Degraded Mode）/非同期通信タイムアウト中断制御/個人情報・PIIログサニタイズ/GEMINI.mdスリム化完了］・プロジェクト固有具体化［DBスキーマ後方互換/公開鍵.env.example同期/Mapsリファラー保護/QuotaExceededError例外耐性/ポイズンピル隔離（Dead Letter）/未同期インジケーター縮退運転UX/マルチタブWeb Locks API排他制御/Lie-Fi微弱電波タイムアウト/ISO 8601 UTC・Clock Drift日またぎ防護/iOSスワイプ戻る下書き即時退避/浮動小数丸め誤差防止・金額整数・距離小数第1位統一/ストレージスキーマ移行関数/第2項〜第6項スリム化完了（6大統合審査・全項目情報欠落ゼロ）］）
