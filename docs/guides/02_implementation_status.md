...
- [x] Google Maps継続利用に伴うセキュリティ防護強化 ＆ AI誤判定防止プロトコル確立（クライアント公開キーモデルの正当性明文化、秘密鍵混同・不要なバックエンド新設提案の封殺、Google Maps 4重防護モデル策定、日次クォータ上限ハードリミット設定・環境分離・予算アラート手順整備完了）
...
1. **iPhone PWA 操作性・堅牢性改修（最優先）**
   - **入力フォーム自動ズーム防止 & 現場UI強化**: `src/pages/LogEntry.jsx` の入力欄を `text-base`（16px）へ修正、業務日（`work_date`）ピッカー、プラットフォーム選択、下書き即時退避＆クリア
   - **即死防止（ErrorBoundary導入）**: `src/App.jsx` または `main.jsx` に例外遮断・フォールバックUIを設置（再読み込み ＆ 未同期データJSON救済導線）
   - **プライバシー＆RLS基盤整備**: `src/lib/storage.js` に `device_id`（端末UUID）の自動採番・付与を追加し、Supabase連携時のデータ保護境界を構築
   - **誤操作防止ガード**: デモデータ投入・全リセット実行時の実データ保護2段階確認モーダル設置
2. **分析・ダッシュボード強化**
