...
2. **iPhone PWA / モバイル操作性基準（配達現場向け絶対要件）**:
   ...
   - **長時間稼働 & 省電力・熱暴走保護**: Screen Wake Lock（非対応安全無視）、ChunkLoadError境界（自動再読込）を常設。WebKitメモリ保護のため `useEffect` 内の購読解除を徹底。ナビ切替等の画面非アクティブ時は `visibilitychange` に連動して不要なタイマー・処理を一時休止し、炎天下の熱暴走とバッテリー急減を防止する。
