...
2. **iPhone PWA / モバイル操作性基準（配達現場向け絶対要件）**:
   ...
   - **長時間稼働 & 省電力・熱暴走保護**: Screen Wake Lock（非対応安全無視）、ChunkLoadError境界（自動再読込）を常設。WebKitメモリ保護のため `useEffect` 解除を徹底。画面非アクティブ時は `visibilitychange` 連動で休止し熱暴走を防止。稼働・走行等の時間計測は `setInterval` 加算を禁止し、実時刻差分（タイムスタンプ比較）から直接算出してスリープ・低電力モード時の集計崩れを完全防止する。
