...
3. **オフラインファースト ＆ データ同期設計（現場データ保全）**:
   ...
   - **スキーマ進化 ＆ 計算精度規約**: 端末キャッシュ破損を防ぐLocalStorageデフォルト値マージを必須化（キー変更時は安全な移行関数を通す）。Supabase変更時も新カラムはNULL許容/DEFAULT値を死守。またJS浮動小数誤差による表示・CSV崩れを防ぐため、金額（売上・時給・単価）は整数丸め（`Math.round`）、距離は小数第1位で端数処理を統一する。
